...
export function ListaPrioridades() {
  ...
  const iniciarEdicao = (prioridade: Prioridade) => {
    ...
    setTextoEditando(prioridade.titulo)
  }

  // Função para salvar edição
  const salvarEdicao = () => {
    if (prioridadeEditando && textoEditando.trim()) {
      editarPrioridade(prioridadeEditando.id, { titulo: textoEditando })
            ...
            >
              {isToday() && (
                <button
                  ...
                  aria-label={prioridade.completada ? 'Marcar como não concluída' : 'Marcar como concluída'}
                >
                  {prioridade.completada ? (
